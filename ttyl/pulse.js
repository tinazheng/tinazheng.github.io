...
const ground = Bodies.rectangle(
  windowWidth / 2,
  windowHeight - boundaryWidth / 2,
  windowWidth,
  groundHeight,
  wallOptions
);
const rightWall = Bodies.rectangle(
  windowWidth - boundaryWidth / 2,
  windowHeight / 2,
  boundaryWidth,
  windowHeight - (boundaryWidth + groundHeight),
  wallOptions
);
...
const shootOutAirpods = () => {
  Body.applyForce(
    airpod1,
    { x: airpod1.position.x, y: airpod1.position.y },
    { x: 1.5, y: -1.5 }
  );
  Body.applyForce(
    airpod2,
    { x: airpod2.position.x, y: airpod2.position.y },
    { x: -0.5, y: -1.0 }
  );
...
const isStill = body =>
  body.speed > 0.0001 && body.speed < STILL_SPEED_THRESHOLD;
const isWithinPositionRange = (first, second) =>
  Math.abs(first.x - second.x) < POS_THRESHOLD &&
  Math.abs(first.y - second.y) < POS_THRESHOLD;
...
function updateAirpodsLocation() {
  if (isLanding) {
    if (isStill(airpod1) && isStill(airpod2)) {
      setTimeout(() => {
        isLanding = false;
        isGoingBack = true;
        [ground, ceiling, rightWall, leftWall].forEach(body =>
  ...
  if (isGoingBack) {
    const ringCoordinates = document
      .getElementById("ring")
    ...
    if (
      isWithinPositionRange(airpod1.position, {
        x: airpod1XPos,
        y: airpod1YPos
      }) &&
      isWithinPositionRange(airpod2.position, {
        x: airpod2XPos,
        y: airpod2YPos
      })
    ) {
      ...
      const velocity1 = {
        x: (airpod1XPos - airpod1.position.x) / 10,
        y: (airpod1YPos - airpod1.position.y) / 10
      };
      const velocity2 = {
        x: (airpod2XPos - airpod2.position.x) / 10,
        y: (airpod2YPos - airpod2.position.y) / 10
      };
  ...
  if (!isLanding && !isGoingBack) {
    ...
    airpodAngleOffset =
      (airpodAngleOffset + angleOffsetPerTimestep) % (2 * Math.PI);
